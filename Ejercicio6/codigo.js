...
//Por tanto si el resto de la división es 0, la letra del DNI es la T y 
//si el resto es 3 la letra es la A. Con estos datos, elaborar un pequeño script que:
...
//o mayor que 99999999. Si ese es el caso, se muestra un mensaje al usuario indicando que el número proporcionado
...
let DNI = Number(prompt("ingrese su numero de dni para poder robarle su identidad.."));
let letra = prompt("ingrese la letra de su dni").toUpperCase();
ejercicio(DNI, letra);
function ejercicio (DNI, letra) {
    if (errDNI(DNI)) {
        console.log("su dni no es correcto");
    }else if (getLet(DNI)!=letra) {
        console.log("Su DNI es:",DNI,"La letra que coloco no correponde a su DNI , su letra es :", getLet(DNI));
    } else {
        console.log("su DNI es:", DNI, "su letra es:",letra);
    }
